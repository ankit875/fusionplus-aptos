...
router.post("/fillOrder", async (req, res) => {
  ...
  if (srcChainId === CHAIN_IDS?.SEPOLIA) {
   ...
   const resolverContract = new EthereumResolverContract(
    ...
    console.log(
      `[Ethereum] Order ${orderHash} filled for ${fillAmount} USDC in tx: ${orderFillHash}`
    );
    ...
    console.log(
      `[Aptos Testnet] Funding destination escrow for order ${orderHash}`
    );
    ...
    const ethereumEscrowEvent =
      await ethereumFactory.getSrcDeployEvent(ethereumDeployBlock);
    ...
    console.log(
      `[Ethereum] Withdrawing funds for resolver from ${srcEscrowAddress}`
    );
...
const secretHashU8 = new Uint8Array(ethers.getBytes(orderHash));
   const orderID = await anounce_order({
    ...
    console.log(
      `[Aptos Testnet] Order ${orderHash} filled for ${makingAmount} USDC with order ID: ${orderID}`
    );
