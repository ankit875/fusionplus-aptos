module myapttoken::my_token {
    ...
    fun test_initialize_and_mint() acquires Capabilities {
        ...
        use aptos_framework::aptos_coin; // Import aptos_coin for test setup
        use aptos_framework::coin; // Import coin for capability destruction

        // Initialize the Aptos coin system for testing
        let aptos_framework = account::create_account_for_test(@aptos_framework);
        let (burn_cap, mint_cap) = aptos_coin::initialize_for_test(&aptos_framework);

        // Explicitly destroy the capabilities
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);

        // Create test accounts
        let admin = account::create_account_for_test(@myapttoken);
